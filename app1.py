...
# ---------------- Page & Style ----------------
st.set_page_config(page_title="Americas Radiopharma OTP Dashboard", page_icon="📊",
...
st.title("Americas Radiopharma OTP Dashboard")
...
AMERICAS_COUNTRIES = {
    ...
    'HT', 'JM', 'TT', 'BB', 'BS', 'AG', 'DM', 'GD', 'KN', 'LC', 'VC', 'PR', 'VI',
    'AI', 'AW', 'BM', 'KY', 'TC', 'VG', 'GL', 'PM', 'BL', 'MF', 'SX', 'CW', 'BQ'
}

# Healthcare keywords for identification (comprehensive list)
HEALTHCARE_KEYWORDS = [
    ...
    'cancer', 'therapy', 'research', 'institute', 'university',
    'biotech', 'biopharma', 'biosimilar', 'biologics', 'antibody',
    'clinical trial', 'cro', 'contract research', 'fda', 'regulatory',
    'medical device', 'implant', 'prosthe', 'dental', 'ophthalm',
    'dermatolog', 'endocrin', 'gastro', 'hemat', 'nephro', 'pulmon',
    'rheumat', 'urology', 'gynecol', 'obstetric', 'pediatric', 'geriatric',
    'emergency', 'trauma', 'intensive care', 'icu', 'surgery', 'anesthe',
    'radiology', 'mri', 'ct scan', 'x-ray', 'ultrasound', 'mammogr',
    'dialysis', 'infusion', 'injection', 'syringe', 'catheter', 'stent',
    'pacemaker', 'defibrillator', 'ventilator', 'monitor', 'diagnostic kit',
    'test kit', 'pcr', 'elisa', 'western blot', 'flow cytometry',
    'mass spec', 'chromatography', 'sequencing', 'genomics', 'proteomics',
    'bioinformatics', 'precision medicine', 'personalized medicine',
    'regenerative', 'gene therapy', 'car-t', 'immunotherapy', 'checkpoint',
    'monoclonal', 'biosafety', 'gmp', 'gcp', 'glp', 'cleanroom',
    'steril', 'aseptic', 'validation', 'qualification', 'calibration',
    # Specific company names and institutions
    'marken', 'fisher', 'cardinal', 'patheon', 'organox', 'qiagen', 'abbott',
    'tosoh', 'leica', 'sophia', 'cerus', 'sirtex', 'lantheus', 'avid',
    'petnet', 'innervate', 'ndri', 'pentec', 'garvan',
...
NON_HEALTHCARE_KEYWORDS = [
    ...
    'tiffany', 'cartier', 'rolex', 'omega', 'tag heuer', 'breitling',
    'automotive', 'ford', 'general motors', 'chrysler', 'tesla', 'toyota',
    'honda', 'nissan', 'volkswagen', 'bmw', 'mercedes', 'audi', 'porsche',
    'ferrari', 'lamborghini', 'mclaren', 'bentley', 'rolls royce',
    'electronics', 'apple', 'samsung electronics', 'lg electronics', 'sony',
    'panasonic electronics', 'sharp', 'toshiba', 'hitachi', 'mitsubishi electric',
    'food service', 'restaurant', 'hotel', 'hospitality', 'travel', 'tourism',
    'entertainment', 'media', 'broadcasting', 'streaming', 'netflix', 'hulu',
    'spotify', 'pandora', 'sirius', 'comcast', 'directv', 'dish network',
    'verizon', 'at&t', 'tmobile', 'sprint', 'cricket', 'metro pcs',
    'banking', 'finance', 'insurance', 'real estate', 'mortgage', 'lending',
    'credit', 'investment', 'trading', 'forex', 'cryptocurrency', 'bitcoin',
    'mining', 'oil gas', 'petroleum', 'energy', 'utility', 'power generation',
    'construction', 'engineering', 'architecture', 'design', 'consulting',
...
def is_healthcare(account_name, sheet_name=None):
    ...
    # Special rule: Aviation Services sheet is always non-healthcare
    if sheet_name == 'Aviation Services':
        return False
...
def read_and_combine_sheets(uploaded):
    """Read Excel and split into healthcare and non-healthcare dataframes for Americas."""
    try:
        # Define all 9 sheets - MUST match exact names in Excel
        all_sheets = ['New York', 'Americas International Desk', 'Atlanta', 'Los Angeles', 
                      'Miami', 'MNX Charter', 'Aviation Services', 'Orange County', 'Time Critical Control']
        
        # Sheets that don't need Americas filtering (already US-based)
        us_based_sheets = ['New York', 'Atlanta', 'Los Angeles', 'Miami', 'Orange County']
        
        # Sheets that need Americas filtering
        filter_sheets = ['Americas International Desk', 'MNX Charter', 'Time Critical Control']
        
        # Aviation Services is special: non-healthcare only, needs Americas filter
        aviation_sheet = 'Aviation Services'
        ...
        for sheet_name in all_sheets:
            try:
                ...
                if sheet_name in us_based_sheets:
                    # These sheets are already US-based, assume all are Americas
                    df_sheet_americas = df_sheet.copy()
                    # But still mark them as US for consistency
...
def create_dashboard_view(df: pd.DataFrame, tab_name: str, otp_target: float, debug_mode: bool = False):
    ...
    if df.empty:
        st.info(f"No {tab_name} data available after filtering for Americas countries and 440-BILLED status.")
        return
    ...
    **{tab_name} Data Processing:**
    ...
    - **Filter**: Americas countries only, STATUS = 440-BILLED
    ...
    if not otp_pod.empty:
        ...
        # Add percentage labels for Gross OTP (above the line)
        for xi, yi in zip(x, gross_y):
            if pd.notna(yi):
                fig2.add_annotation(
                    ...
                    text=f"<b>{yi:.2f}%</b>",
                    showarrow=False,
                    yshift=15,  # Above the line
                    font=dict(size=12, color=BLUE),
                    bgcolor="rgba(255,255,255,0.8)"
                )
        
        # Add percentage labels for Net OTP (also above the line)
        for xi, yi in zip(x, net_y):
            if pd.notna(yi):
                fig2.add_annotation(
                    ...
                    text=f"<b>{yi:.2f}%</b>",
                    showarrow=False,
                    yshift=15,  # Above the line (same as Gross)
    ...
    # ---------------- Chart: 10 Worst Accounts by Net OTP — August ----------------
    st.subheader(f"{tab_name}: 10 Worst Accounts by Net OTP — August")
        ...
        if not base.empty:
            ...
            if not aug_df.empty:
                ...
                if not aug_latest.empty:
                    ...
                    if not grp.empty:
                        # Changed from 5 to 10 worst accounts
                        worst = grp.nsmallest(10, 'Net_OTP').copy()
                        ...
                        figw.update_layout(
                            title_text=f"August {latest_aug_year} — Worst 10 by Net OTP (with Volume)",
                            height=480,
                            plot_bgcolor="white",
                        ...
                        st.caption(f"Worst 10 accounts — August {latest_aug_year} (Net OTP and Volume)")
                        st.dataframe(
...
if not uploaded_file:
    ...
    **Expected file structure:**
    - 9 sheets: New York, Americas International Desk, Atlanta, Los Angeles, Miami, 
      MNX Charter, Aviation Services, Orange County, Time Critical Control
...
if debug_mode:
    with st.expander("🔍 Debug: Complete Data Flow"):
        ...
        for sheet_name in ['New York', 'Americas International Desk', 'Atlanta', 'Los Angeles', 
                          'Miami', 'MNX Charter', 'Aviation Services', 'Orange County', 'Time Critical Control']:
        ...
        st.write("\n**2. Combined Data:**")
        st.write(f"   Total combined (Americas + 440-BILLED): {stats.get('status_filtered', 0):,} rows")
...
with st.expander("📈 Data Processing Statistics"):
    ...
    with col2:
        st.metric("Americas Rows", f"{stats.get('americas_rows', 0):,}")
    with col3:
